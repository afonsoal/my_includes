...
void NewMesh::set_variables(const std::vector<Point<2> > & _support_points,
...
void NewMesh::create_new_triangulation (Triangulation<2> &tria)
{
	std::vector<CellData<2> > cells_data (vector_new_cell_struct.size(), CellData<2>());
...
void NewMesh::set_triangulation_indices (Triangulation<2> &tria) {
	int count_cell = 0;
